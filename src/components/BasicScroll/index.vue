...
import getDomWH from '@/hooks/domWH'
...
export default {
  ...
  setup (props) {
    ...
    const { width, height } = getDomWH(id)
    const actualConfig = ref({})
    const actualHeaderData = ref([])
    const actualHeaderStyle = ref([])
    const actualRowData = ref([])
    const currentRowData = ref([])
    const actualRowStyle = ref([])
    ...
    const handleHeader = () => {
      ...
      actualRowData.value = _rowData.map((item, index) => ({
        data: item,
        index: index
      }))
    }
